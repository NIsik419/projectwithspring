...
> 조건: 50 VU, 30s / 총 요청: 11,870 / 처리량: ~385 req/s / 에러율: 0.00%

| API | avg (ms) | p(95) ms | p(99) ms | max (ms) | 목표 p95 | 통과 |
|-----|----------|----------|----------|----------|----------|------|
| GET /cafe/search | 5.32 | **20.29** | 44.28 | 380.95 | < 200ms | ✅ |
| GET /cafe/name-search | 8.55 | **27.38** | 55.9 | 312.03 | < 200ms | ✅ |
| POST /auth/login | 86.55 | **105.3** | 136.05 | 297.6 | < 200ms | ✅ |

> ⚠️ **주의**: 측정 당시 `cafe_aspect_vectors` 데이터 없음 → 캐시 0개 로드됨.
> `/cafe/search` 수치는 **빈 배열 즉시 반환** 케이스이므로, 실 데이터(304개) 환경에서 재측정 필요.
> `/cafe/name-search`도 동일 이유로 실 데이터 환경 재측정 권장.
> `/auth/login` 수치는 실 부하와 동일 (JWT 발급 포함).
